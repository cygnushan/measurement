...
class RT_Canvas(FigureCanvas):
    ...
    def plot(self, datax, datay, log, style, type):
        if type == 1:
            if log:
                self.curveObj,= self.ax.semilogy(datax,datay,style)
            else:
...
class  RT_CanvasWidget(QtGui.QWidget):
    ...
    def generateData(self,x,y,log, style='b', type=0):
        self.canvas.plot(x,y,log, style,type)   
