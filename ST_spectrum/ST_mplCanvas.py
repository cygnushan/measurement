...
class ST_Canvas(FigureCanvas):
    def __init__(self):
        ...
        self.ax.set_title(u"S-T谱")
        self.ax.set_xlabel(u"温度(℃)",labelpad=0)
    ...
    def set_ylog(self, islog, ytype):
        if islog:
            self.ax.set_ylabel(u"灵敏度(logS)",labelpad=0)       
        self.draw()

    def plot(self, datax, datay, style, type):
        if type == 1:
            self.curveObj,= self.ax.plot(datax,datay,style)
...
class  ST_CanvasWidget(QtGui.QWidget):
    ...
    def generateData(self,x,y, style='b', type=0):
        self.canvas.plot(x,y,style,type)   
